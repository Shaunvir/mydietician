...
class InsuranceCardScanner {
    ...
    async initializeMobileWebRTC() {
        try {
            console.log('Initializing mobile direct connection...');
            
            // Store session for later use
            this.mobileSessionReady = true;
            this.updateMobileStatus('connected', '📡 Connected to desktop - ready to scan');
            
            console.log('Mobile connection initialized via direct method');
    ...
    // Removed WebRTC-specific functions - using direct submission approach instead
    ...
    useExtractedData() {
        ...
        // Check if we're in mobile mode - send data via direct submission
        if (this.isMobileMode && this.mobileSessionReady) {
    ...
    sendDataToDesktop() {
        try {
            ...
            // Prepare data with security timestamp
            const timestamp = Date.now();
            const dataWithSecurity = {
                ...this.extractedData,
                _timestamp: timestamp,
            ...
            // Store data in localStorage for desktop to retrieve
            localStorage.setItem('crossDeviceScanData', JSON.stringify(dataWithSecurity));
            localStorage.setItem('crossDeviceScanSession', this.sessionId);
            localStorage.setItem('crossDeviceScanTimestamp', timestamp.toString());
            
            // Create desktop redirect URL
            const protocol = window.location.protocol;
            const host = window.location.host;
            const desktopUrl = `${protocol}//${host}/search.html?mobileComplete=${this.sessionId}&timestamp=${timestamp}`;
            ...
            if (actionButtons) {
                actionButtons.parentElement.innerHTML = `
                    <div style="text-align: center; padding: 16px; background: #f0fdf4; border-radius: 8px; color: #059669;">
                        ...
                        <small>Redirecting you back to desktop...</small>
                    </div>
                `;
            }
            
            // Redirect to desktop after brief delay
            setTimeout(() => {
